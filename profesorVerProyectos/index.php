...
if ($resultado1 = $mysqli->query("SELECT * FROM proyectos WHERE profesor='$nom' GROUP BY nombreproyecto")) {
...
echo "</form>";
echo "<form method='POST' action='http://localhost/moodle/local/profesorAsignarProyectos/index.php'>";
$codigo=$_POST['select1'];
echo "<input type='hidden' name='cod' value='$codigo'>";
if($descripcion1= $mysqli->query("SELECT * FROM proyectos WHERE profesor='$nom' AND codigo='$codigo' GROUP BY profesor" ))
{
